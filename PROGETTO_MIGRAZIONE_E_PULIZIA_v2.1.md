...
```
nul                             # File vuoto (errore Windows - vedi nota)
*.log                           # Log vuoti
...
**⚠️ NOTA TECNICA: Il File `nul`**

Il file `nul` è un **bug noto di Windows** che si manifesta quando:
1. Script o comandi tentano di reindirizzare output a `nul` (device null di Windows)
2. Viene creato accidentalmente un file letterale chiamato "nul" invece di usare il device

**Problema:** Windows non permette la normale eliminazione di file chiamati `nul`, `con`, `prn`, `aux`, `com1-9`, `lpt1-9` (nomi riservati MS-DOS).

**Come si crea (esempio):**
```bash
# Comando errato che crea il file invece di usare il device
echo "test" > nul  # Invece di > NUL o > $null in PowerShell
```

**Come eliminarlo:**

```powershell
# Metodo 1: Usare path UNC
Remove-Item "\\?\C:\Users\marin\Downloads\TournamentMaster\nul" -Force

# Metodo 2: Da CMD (non PowerShell)
del "\\?\C:\Users\marin\Downloads\TournamentMaster\nul"

# Metodo 3: Linux Subsystem (WSL)
wsl rm /mnt/c/Users/marin/Downloads/TournamentMaster/nul
```

**Perché è importante rimuoverlo:**
- Blocca operazioni di copia/spostamento ricorsive
- Causa errori nei backup automatici
- Impedisce eliminazione cartella padre
- Confonde Git e altri version control
...
# Se trova il file, lo script lo eliminerà automaticamente
```

Lo script di migrazione gestisce automaticamente questo file usando la sintassi `\\?\` per bypassare le restrizioni di Windows.
...
### 5.8 Variabili d'Ambiente Windows (PATH)

**Status:** ⚠️ Verifica raccomandata dopo migrazione

**Controllo necessario:** Verificare che nessun tool globale o script punti esplicitamente al path originale.

**Come verificare:**

```powershell
# 1. Aprire Variabili d'Ambiente
# Win + Pause → Impostazioni di sistema avanzate → Variabili d'ambiente

# 2. Verificare PATH utente e sistema
$env:PATH -split ';' | Select-String "TournamentMaster"

# Se trova risultati con il vecchio path:
# C:\Users\marin\Downloads\TournamentMaster\...
# Devi aggiornarli manualmente con:
# D:\Dev\TournamentMaster\...
```

**Casi comuni da verificare:**

| Tool | Path Tipico | Azione |
|------|-------------|--------|
| npm global scripts | `%APPDATA%\npm` | ✅ Auto-gestito (OK) |
| Custom batch scripts | `C:\Users\...\scripts` | ⚠️ Verificare manualmente |
| IDE configurations | VS Code, WebStorm | ⚠️ Aggiornare workspace |
| Windows Scheduler | Task pianificati | ⚠️ Aggiornare path assoluti |

**Nota:** Con npm e Node.js installati correttamente, i package globali NON contengono path assoluti al progetto, quindi questo raramente è un problema. Tuttavia, se hai script personalizzati o automazioni, controlla attentamente.

**Verifica rapida:**

```powershell
# Cercare riferimenti al vecchio path in file di configurazione comuni
$searchPath = "C:\Users\marin"
$oldProjectPath = "Downloads\TournamentMaster"

# PowerShell profile
Select-String -Path "$env:USERPROFILE\Documents\PowerShell\Microsoft.PowerShell_profile.ps1" `
    -Pattern $oldProjectPath -ErrorAction SilentlyContinue
...
# Git config globale
git config --global --get-regexp "TournamentMaster"
```

### 5.9 Docker File Sharing (Preparazione Futura)

**Status:** 📋 Non necessario ora, ma importante per deployment futuro

**Contesto:** Se in futuro decidi di usare Docker per containerizzare TournamentMaster (come pianificato nelle specifiche), il percorso `D:\` su Windows richiede configurazione esplicita.

**Azione post-migrazione (quando userai Docker):**

1. **Docker Desktop → Settings → Resources → File Sharing**
2. Aggiungere esplicitamente `D:\` alla lista di drive condivisi
3. Riavviare Docker Desktop

**Perché è necessario:**

Windows e Docker Desktop (con WSL2) hanno restrizioni su quali drive possono essere montati nei container. Di default, solo `C:\Users\` è condiviso automaticamente.

**Comando test (futuro):**

```bash
# Testare che Docker possa accedere a D:\Dev
docker run --rm -v D:/Dev/TournamentMaster:/app alpine ls -la /app

# Se funziona, vedrai la lista dei file del progetto
# Se fallisce, devi aggiungere D:\ al file sharing
```

**Configurazione Docker Compose (futura riferimento):**
...
services:
  backend:
    build: ./backend
    volumes:
      - D:/Dev/TournamentMaster/backend:/app  # Path assoluto Windows
      # Oppure relativo se docker-compose.yml è in root:
      - ./backend:/app
```

**Nota:** Questa configurazione NON è necessaria per la migrazione attuale. È documentata qui per riferimento futuro quando implementerai Docker deployment come da specifiche tecniche.
...
# File nul (bug Windows - usa path UNC per bypassare restrizioni)
$nulPath = "\\?\$dest\nul"
if (Test-Path $nulPath) {
    if (-not $DryRun) { 
        Remove-Item $nulPath -Force 
        Write-Host "  Eliminato: file 'nul' (bug Windows)" -ForegroundColor Green
    }
    $deleted++
}
...
### Verifiche Avanzate (Opzionali ma Raccomandate)
- [ ] **PATH:** Verificato che variabili d'ambiente non contengono riferimenti al vecchio path
  - `$env:PATH -split ';' | Select-String "TournamentMaster"`
- [ ] **PowerShell Profile:** Nessun alias/funzione punta al vecchio path
  - `Select-String -Path $PROFILE -Pattern "Downloads\\TournamentMaster"`
- [ ] **VS Code Settings:** Workspace non contiene path assoluti obsoleti
  - Controllare `.vscode/settings.json`
- [ ] **Git Config:** Nessuna configurazione locale con path assoluti
  - `git config --list --local`
- [ ] **Docker Desktop:** D:\ aggiunto a File Sharing (se/quando userai Docker)
  - Settings → Resources → File Sharing → Add `D:\`
- [ ] **File `nul`:** Verificato assenza in nuova location
  - `Test-Path "\\?\D:\Dev\TournamentMaster\nul"` → False
- [ ] **Symlinks:** Nessun link simbolico punta al vecchio path
  - `Get-ChildItem D:\Dev\TournamentMaster -Recurse | Where-Object LinkType`
...
**Versione:** 2.1 (PWA-Only Aligned + Advanced Checks)  
**Ultima modifica:** 2026-01-06 02:15  
**Changelog:**
...
- **v2.1: Aggiunte verifiche avanzate - variabili PATH Windows, preparazione Docker File Sharing, gestione file `nul` con nota tecnica dettagliata, checklist verifiche avanzate**
